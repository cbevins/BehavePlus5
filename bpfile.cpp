...
void BpDocument::save( const QString &fileName, const QString &fileType )
{
    ...
    else if ( fileType == "Export Fuel Farsite" )
    {
        saveAsFuelModelExportFile( "Farsite" );
    }
 ...
 *
 *  \param fileType One of "Farsite" or "FlamMap".
 */
...
void BpDocument::saveAsFuelModelExportFile( const QString &fileType )
{
    ...
    fprintf( fptr, "ENGLISH\n" );
    //fprintf( fptr, "# fuel fuel    1-h  10-h 100-h  live  live    fuel   1-h  live  live  fuel dead    dead    live fuel\n" );
    //fprintf( fptr, "#model model  fuel  fuel  fuel  herb woody   model  dead  herb woody   bed fuel    heat    heat model\n" );
    //fprintf( fptr, "# numb code   load  load  load  load  load    type   SAV   SAV   SAV depth   Mx content content name\n" );
    QString fmName;
    FuelModel *fmPtr;
    int custom = 14;
    ...
    {
        fmName = parser.token(i);
        if ( ! ( fmPtr = fuelModelList->fuelModelByModelName( fmName ) ) )
        // This code block should never be executed!
        {
            QString text("");
            translate( text, "FuelExport:FindModelName:Error", fmName );
            warn( text );
            return;
        }
        // Assign number to the custom fuel model
        int number = fmPtr->m_number;
        if ( number == 0 )
        {
            number = custom++;
        }
        // Write the fuel record
        if ( fileType == "Farsite"
          || fileType == "Flammap" )
        {
            fprintf( fptr,
                " %5d %-16s "                       // Number, code
                "%5.2f %5.2f %5.2f %5.2f %5.2f "    // 5 loads
                "%7.7s %5.0f %5.0f %5.0f "          // Type, 3 SAV's
                "%5.2f %4.0f %7.1f %7.1f %s\n",     // Depth, Mx, 2 Heats, Desc
                number,
                fmPtr->m_name.latin1(),
                fmPtr->m_load1 * 21.78,
                fmPtr->m_load10 * 21.78,
                fmPtr->m_load100 * 21.78,
                fmPtr->m_loadHerb * 21.78,
                fmPtr->m_loadWood * 21.78,
