 ...
 *  \version BehavePlus3
 *  \author Copyright (C) 2004 by Collin D. Bevins.  All rights reserved.
 *
...
void BpDocument::composeContainDiagram( void )
{
          ...
          i++ )
    {
        ...
        // Y-scala could be based on either constructed line or attack perim
        yValMax = ( val[YMAX][i] > yValMax ) ? val[YMAX][i] : yValMax;
        double l = val[ABACK][i] + val[AHEAD][i];
        double y = l / val[RATIO][i];
        yValMax = ( y > yValMax ) ? y : yValMax;
    }
    ...
    niceAxis(  xValMin, xValMax, 10, &xMin, &xMax, &xTics, &xStep, &xDec );
    niceAxis( -yValMax, yValMax, 10, &yMin, &yMax, &yTics, &yStep, &yDec );

    // Determine graph WORLD coordinates within figHt and figWd
    // (diagram must have a 1:1 aspect ratio since it is a map)
    double gXMin, gXMax, gYMin, gYMax, w, h, l_x, l_y, z, l, fx, fy, fz;
    double aspect = figHt / figWd;
    if ( aspect < ( (  yMax - yMin ) / ( xMax - xMin ) ) )
    {
        gYMin = yMin - yStep;           // diagram WORLD maximum Y
        gYMax = yMax + yStep;           // diagram WORLD minimum Y
    ...
    {
        gXMin = xMin - xStep;           // diagram WOLD maximum X
        gXMax = xMax + xStep;           // diagram WORLD minimum X
