    ...
    m_lvi(0)
{
    ...
    translate( text, "FuelExportDialog:Col0" );
    m_listView->addColumn( text );
    translate( text, "FuelExportDialog:Col1" );
    m_listView->addColumn( text );
    translate( text, "FuelExportDialog:Col2" );
    m_listView->addColumn( text );
    if ( showSortCol )
    {
    ...
    for ( int iid = 0;
          ...
          iid++ )
    {
        desc = *( varPtr->m_itemList->itemDesc( iid ) );
        item = new QListViewItem( m_listView,
            varPtr->m_itemList->itemSort( iid ),
            varPtr->m_itemList->itemName( iid ),
            desc );
        Q_CHECK_PTR( item );
    }
