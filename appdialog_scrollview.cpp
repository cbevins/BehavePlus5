...
AppPage::AppPage( QWidget *p_parent, const QString &pictureFile,
    ...
    m_showPicture(0)
{
	// V5.0.5 adds scrollbars!
	// The QSplitter is the sole child of this scrollview
    m_splitter = new QSplitter( this, QString("m_splitter%1").arg( p_name ) );
    Q_CHECK_PTR( m_splitter );
    addChild( m_splitter );
    // Add some visual space around the borders.
    m_splitter->setMargin( 10 );

    // The optional left pane contains a picture.
    if ( ! pictureFile.isNull() && ! pictureFile.isEmpty() )
    {
        // Create a label containing the picture.
        m_pictureLabel = new QLabel( m_splitter,
    ...
    m_midFrame = new QVBox( m_splitter, "m_midFrame" );
    ...
    if ( ! htmlFile.isNull() && ! htmlFile.isEmpty() )
    {
        ...
        m_helpBrowser = new HelpBrowser( m_splitter, htmlDir, htmlFile, indexFile,
...
AppPage::~AppPage( void )
{
	...
	delete m_splitter;			m_splitter = 0;
    return;
}

//------------------------------------------------------------------------------
/*! \brief Called by the client after constructing all the page's contents
 *	so the scrollview size can be adjusted.
 *
 *  \param width Initial width; if 0 (default), then a size hint is applied.
 *  \param height Initial height; if 0 (default), then a size hint is applied.
 *  \param x Initial left-hand pixel (default==0).
 *  \param y Initial top pixel (default==0).
 */
