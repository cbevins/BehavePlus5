...
void AppWindow::openStartups( void )
{
    ...
    if ( ! status )
    {
        ...
        // default definition in BehavePlus5.xml, so it needs a path
...
void AppWindow::initMenuFile( void )
{
    ...
    // Fuel models submenu
    {
        ...
        // File->Save as fuel model->FARSITE format
        translate( text, "Menu:File:Export:FuelModels:Farsite" );
        m_idFileExportFuelModelsFarsite = m_fileExportFuelModelsMenu->insertItem(
            text, this, SLOT( slotDocumentExportFuelModelsFarsite() ) );
...
void AppWindow::setLanguage( const QString &language )
{
    ...
    translate( text, "Menu:File:Export:FuelModels:Farsite" );
    menuBar()->changeItem( m_idFileExportFuelModelsFarsite, text );
...
//------------------------------------------------------------------------------
/*! \brief Exports selected fuel models to a FARSITE-formatted file.
 *
 *  Called only by the \b File->Export->FuelModels->FARSITE menu selection.
 */

void AppWindow::slotDocumentExportFuelModelsFarsite( void )
{
    log( "Beg Section: AppWindow::slotDocumentExportFuelModelsFarsite() invoked ...\n" );
    Document *doc = getActiveWindow( "BpDocument" );
    if ( doc )
    {
        log( QString( "Exporting FARSITE fuel models for document \"%1\" ...\n" )
            .arg( doc->m_absPathName ) );
        // Passing fileName==0 forces a file dialog.
        doc->save( 0, "Export Fuel Farsite" );
    }
    log( "End Section: AppWindow::slotDocumentExportFuelModelsFarsite() completed.\n" );
    return;
}
...
void AppWindow::slotStatusUpdate( void )
{
      ...
      || (windows.count() == 1 && windows.at(0)->caption().isNull() ) )
    {
        ...
        m_fileMenu->setItemEnabled( m_idFileExportFuelModelsFarsite, false );
    ...
    else
    {
        ...
        m_fileMenu->setItemEnabled( m_idFileExportFuelModelsFarsite, true );
